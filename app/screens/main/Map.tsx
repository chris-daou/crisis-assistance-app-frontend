...
export default function MapScreen() {
  ...
  // 2) Socket.io + initial friend locations
  useEffect(() => {
    let isMounted = true;
    (async () => {
      ...
      const userId = (await AsyncStorage.getItem('user'))?.replace(/"/g,'') || '';
      try {
        const resp = await api.get<{friends:any[]}>(
          'user/social/connected-locations',
          { headers: { 'user-id': userId, Authorization: `Bearer ${token}` } }
        );
        const mapped: FriendLocation[] = resp.data.friends.map(f => ({
          userId: f._id,
          name:   f.name,
          lat:    Number(f.location.lat),
          lng:    Number(f.location.lng),
          color:  `hsl(${Math.floor(Math.random()*360)},70%,80%)`,
        }));
        if (isMounted) setFriendLocations(mapped);
      } catch (err) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {places.map((place, i) => {
          ...
          return (
            <Marker
              key={`${place.name}-${coords.latitude}-${coords.longitude}`}
              coordinate={coords}
...
const styles = StyleSheet.create({
  ...
  customNavButton:       {
    marginTop:10, backgroundColor:'#f4f4f4',
  ...
  customCallout:         {
    width:'90%', padding:15, backgroundColor:'white',
    borderRadius:10, alignItems:'center', elevation:5,
  },
  ...
  menuButtonContainer:    { position:'absolute',top:36,right:10,zIndex:3 },
